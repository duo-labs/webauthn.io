...
func (ms *ModelsSuite) TestGetRelyingPartyByHost() {
	rp, err := GetRelyingPartyByHost(ms.config.HostAddress)
	if err != nil {
		ms.T().Fatalf("Unexpected error getting relying party by hostname: %s", err)
	}
	if rp.ID != config.Conf.HostAddress {
		ms.T().Fatalf("Unexpected RP received. Expected %s, Got %s", ms.config.HostAddress, rp.ID)
	}
...
func (ms *ModelsSuite) TestPutRelyingParty() {
	...
	got, err := GetRelyingPartyByHost(ms.config.HostAddress)
	if err != nil {
		ms.T().Fatalf("Unexpected error getting relying party by hostname: %s", err)
	}
